...
MODEL_PROVIDER = None
# 管理后台注入：model_id -> 上游目录元数据（上下文长度 / 思考档位 / 倍率等）。
MODEL_META = None
...
def list_models(
    ...
    meta = MODEL_META() if MODEL_META is not None else {}
    data = []
    for m in models:
        row = {"id": m, "object": "model", "created": 1700000000, "owned_by": "codebuddy"}
        info = meta.get(m) if isinstance(meta, dict) else None
